 ...
 */
public class Sample4PlayRTC  extends BasePlayRTC{
	 ...
	 */
	public Sample4PlayRTC(BaseActivity activity, PlayRTCObserver oberver) 
	 ...
	 */
	public void setConfiguration(View screenCastView, int resolutionWidth, int resolutionHeight) {
		 ...
		 *  주의를 요함. 
		 */
		settings.setVideoEnable(true); /* 영상 전송 사용 */
		settings.video.setFrontCameraEnable(false); // default true
		settings.video.setBackCameraEnable(false);  // default false
		// target view 
		settings.video.setScreenCastView(screenCastView, resolutionWidth, resolutionHeight);
        ...
        File logPath = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                    "/Android/data/" + activity.getPackageName() + "/files/log/sample4/");  
        ...
        File cachePath = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                "/Android/data/" + activity.getPackageName() + "/files/cache/sample4/");  
