 ...
 */
public class Sample1PlayRTC extends BasePlayRTC{
	 ...
	 */
	public Sample1PlayRTC(BaseActivity activity, PlayRTCObserver oberver) 
	...
	public void setConfiguration() {
		...
		/**
		 * PlayRTC에서 PlayRTCSettings 인스턴스를 얻어온다.
		 */
		...
		settings.video.setFrontCameraEnable(true); // default true
		settings.video.setBackCameraEnable(true);  // default false
        ...
        File logPath = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                    "/Android/data/" + activity.getPackageName() + "/files/log/sample1/");  
        ...
        File cachePath = new File(Environment.getExternalStorageDirectory().getAbsolutePath() +
                "/Android/data/" + activity.getPackageName() + "/files/cache/sample1/");  
